...
def B_batch(x, grid, k=0, extend=True):
    ...
    value = torch.nan_to_num(value).to(device)
    return value
...
def coef2curve(x_eval, grid, coef, k):
    ...
    -----
        ...
        coef : 3D torch.tensor)
            shape (in_dinm, out_dim, number of splines). number of coef params = number of grid intervals + k
        k : int
            the piecewise polynomial order of splines.
        device : str
            devicde
...
def curve2coef(x_eval, y_eval, grid, k):
    ...
    assert x_eval.device == y_eval.device, 'x, y not on the same device!'
