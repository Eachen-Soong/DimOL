...
parser.add_argument('--channel_mixing', type=str, default='prod-layer') #####
...
parser.add_argument('--log_path', type=str, default='./ckpt')
...
model = FNO(in_channels=in_channels, n_modes=(n_modes, n_modes), hidden_channels=args.hidden_channels, lifting_channels=args.lifting_channels,
             projection_channels=args.projection_channels, n_layers=args.n_layers, factorization=args.factorization, channel_mixing=args.channel_mixing, rank=args.rank)
...
file_name = f'{args.data_name}_{args.model_name}_{args.prefix}'
config_name = ''
if args.config_details:
    config_name = f'_layer{args.n_layers}_fact-{args.factorization}_rank{args.rank}_mix-{args.channel_mixing}_prod{args.num_prod}_pos-enc-{args.pos_encoding}_loss{args.train_loss}_mode{args.n_modes}_hid{args.hidden_channels}_lift{args.lifting_channels}_proj{args.projection_channels}_b{args.batch_size}_lr{args.lr}_wd{args.weight_decay}_sche-step{args.scheduler_steps}_gamma{args.scheduler_gamma}'
time_name = ''
if args.time_suffix:
...
log_dir = args.log_path + file_name
save_dir = args.save_path + file_name + '.pth'
# log_dir = args.log_path
#     if log_dir[-1]!='/': log_dir = log_dir + '/'
#     log_dir = log_dir + file_name
#     save_dir = args.save_path
#     if save_dir[-1]!='/': save_dir = save_dir + '/'
#     save_dir = save_dir + file_name + '.pth'
